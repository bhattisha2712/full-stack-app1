...
export default function SystemTestPage() {
  ...
  const runSystemTests = async () => {
    ...
    addTestResult({
      ...
      details: session?.user as Record<string, unknown>
    });

    // Test 3: User Role
    addTestResult({
      name: "User Role Check",
      status: (session?.user as unknown as { role?: string })?.role ? "pass" : "fail",
      message: `Role: ${(session?.user as unknown as { role?: string })?.role || "No role found"}`,
      details: { role: (session?.user as { role?: string })?.role }
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            {session && (
              <p className="text-xs text-blue-600">
                Role: {(session.user as { role?: string })?.role || "No role"}
              </p>
            )}
          </div>
      ...
      {(session?.user as { role?: string })?.role === "admin" && users.length > 0 && (
