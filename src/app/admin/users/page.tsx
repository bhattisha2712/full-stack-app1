...
function RoleControl({ user, onUpdate, userSession }: RoleControlProps) {
  const [updating, setUpdating] = useState(false);
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const [pendingRole, setPendingRole] = useState<string>("");
  // Success state is now managed by parent
  // ...existing code...
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (user.role === "admin" && newRole === "user") {
      ...
      // Do not reset select value here; let modal handle it
      return;
    }
  ...
  return (
    <>
      <div>
        ...
        {user._id === userSession?.user?.id && user.role === "admin" && (
...
function Page() {
  ...
  const handleRoleUpdate = (userId: string, newRole: string) => {
    ...
    setTimeout(() => setSuccessUserId(null), 1200);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Success Alert */}
        {successUserId && (
          <div className="mb-4 bg-green-50 border border-green-200 rounded-lg p-3 text-green-800 text-base shadow-sm animate-fade-in">
      ...
      <tr key={user._id} className="hover:bg-gray-50 transition-colors duration-150">
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          <div className="flex items-center space-x-3">
            <RoleControl user={user} onUpdate={handleRoleUpdate} userSession={session} />
